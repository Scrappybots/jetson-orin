...
boot-image/bootc.ks: boot-image/bootc.ks.tpl auth.json
	@IMAGE=$(IMAGE) AUTH='$(strip $(file < auth.json))' SSH_KEYS='$(SSH_KEYS)' envsubst '$$IMAGE,$$AUTH,$$SSH_KEYS' < $< >$@
